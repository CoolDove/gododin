...
// some functions for temporary usage

string_make :: proc(str: cstring) -> String {
	gstr : String
	gde.string_new_with_utf8_chars(&gstr, str)
	return gstr
}
...
string_to :: proc(gstr: ^String, allocator:=context.allocator) -> cstring {
	context.allocator = allocator
	length := gde.string_to_utf8_chars(gstr, nil, 0)
	cstr := make([]u8, length)
